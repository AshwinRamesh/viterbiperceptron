...
from viterbiperceptron.perceptron import FeaturePerceptronTwo as PerceptronTwo
...
if process == 0:
    ...
    for doc in docs:
        training_data = training_data + doc
    print len(training_data)
    perceptron = PerceptronTwo(training_data=training_data, iterations=3)
    perceptron.train()

    f = file("out.eng.train3", "w+")
    f.write(json.dumps(perceptron.weights))
    f.close()
    print "done"